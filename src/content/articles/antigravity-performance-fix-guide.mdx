...
> [!IMPORTANT]
> **【最短10秒】今すぐ動作を軽くするための3大チェック**
> 1. **New Chatで作り直す**: 履歴の肥大化が原因の9割です。
> 2. **不要なファイルを閉じる**: 右側のContextパネルから不要なファイルを外してください。
> 3. **ブラウザを再起動する**: Chromeのメモリ消費をリセットするだけで劇的に速くなります。
...
> [!TIP]
> **【最短10秒】今すぐ動作を軽くするための3大チェック**
> 1. **チャットの「New Chat」化**: 履歴が長いとAIの思考が失速します。新しいチャットで再開してください。
> 2. **ブラウザの再起動**: Chromeのメモリ消費が原因の場合、再起動だけで劇的に改善します。
> 3. **ファイル添付の最小化**: 不要なファイルをチャットから外すか、`.gitignore` で除外してください。
